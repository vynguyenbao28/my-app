...
class _SwipePullButtonState extends State<SwipePullButton> with TickerProviderStateMixin {
  ...
  Future<void> _animToCoverCell() async {
    ...
    whenActiveToOffset = false;
    animation =
        Tween<double>(begin: offsetX, end: widget.trailing ? -data.contentWidth : data.contentWidth).animate(widthPullCurve)
          ..addListener(() {
            if (lockAnim) return;
            offsetX = animation.value;
            setState(() {});
  ...
  void _animToCoverPullActionContent() async {
    ...
    if (action.nestedAction?.nestedWidth != null && action.nestedAction!.nestedWidth! > data.totalActionWidth) {
      data.parentState
          .adjustOffset(offsetX: action.nestedAction!.nestedWidth!, curve: action.nestedAction!.curve, trailing: widget.trailing);
    }
  ...
  void _initAnim() {
    ...
    if (widget.actionIndex == 0) {
      ...
      offsetFillActionContentCurve =
          CurvedAnimation(parent: offsetFillActionContentController!, curve: action.nestedAction?.curve ?? Curves.easeOutQuart);
    }
  }
