...
class _SwipePullAlignButtonState extends State<SwipePullAlignButton> with TickerProviderStateMixin {
  ...
  void _pullActionButton(bool isPullingOut) {
    ...
    if (isPullingOut) {
      var tween =
          AlignmentTween(begin: alignment, end: trailing ? Alignment.centerLeft : Alignment.centerRight).animate(alignCurve);
      tween.addListener(() {
        if (lockAnim) return;
      ...
      var tween =
          AlignmentTween(begin: alignment, end: trailing ? Alignment.centerRight : Alignment.centerLeft).animate(alignCurve);
      tween.addListener(() {
        if (lockAnim) return;
        alignment = tween.value;
        setState(() {});
  ...
  void _animToCoverPullActionContent() async {
    ...
    if (action.nestedAction?.nestedWidth != null && action.nestedAction!.nestedWidth! > data.totalActionWidth) {
      data.parentState
          .adjustOffset(offsetX: action.nestedAction!.nestedWidth!, curve: action.nestedAction!.curve, trailing: trailing);
    }
